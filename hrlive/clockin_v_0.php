    ...
    if(isset($_POST['Message']) AND $_POST['Message']=='Recent Clock'){
        ...
        if(numRows($sql)==0){
            ...
            if(numRows($sql)>0){
                ...
                // *** check if user haven't clockout from the last request
                $sql = runQuery("SELECT dclockout_img FROM hr_attendance_clock WHERE account='$companyId' AND staffcode='$staffcode' AND dclockout_img IS NULL ORDER BY id DESC LIMIT 1 ");
                ...
                if(numRows($sql)==0){    
                    ...
                    $data = [
                        ...
                        'success'=>false, 
                        'type'=>'clockout',
                        'msg'=>'You haven\'t clockout your recent clockin, make sure you clockout before clocking in.'
                    ];
                }
